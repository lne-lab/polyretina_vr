...
{
	public abstract class FoveInterfaceBase : Singleton<FoveInterfaceBase>
	{
		...
		[Tooltip("Locks orientation (disabling orientation disables rendering")]
		[SerializeField] protected bool lockOrientation = true;
		...
		protected void EnsureLocalDataConcurrency()
		{
			...
			if (_isAuthoritative)
			{
				...
				if (orientation)
				{
					// Code modified by Enrico Migliorini
					if (!lockOrientation)
					{
